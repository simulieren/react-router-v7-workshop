import type { Route } from "./+types/action-example-zod";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const filteredPokemon = validatedParams.search 
    ? data.results.filter((p: {name: string}) => p.name.toLowerCase().includes(validatedParams.search))
    : data.results;
...
export async function action({ request }: Route.ActionArgs) {
  ...
  return { search: validatedParams.search };
}
  ...
  return (
    <div className="p-8">
      ...
      <Form 
        method="get"
        className="mb-8"
        // NOTE: Can change this to onChange if you want to see updates in the URL
